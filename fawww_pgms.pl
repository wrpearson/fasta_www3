...
#	 use_query1 => 1,
	 ...
	 opts => {
	     ...
	     comp_stat => { cmd_arg => "-t %s", val=> \&get_safe_word},
...
#	     msa_asn_rid => { cmd_arg => qq(-q 2 -R %s), val=> \&get_rid2file},
	 ...
	 opts => {
		  ...
		  int_mask => { cmd_arg => "--int_mask %s", val => \&get_safe_word},
		  end_mask => { cmd_arg => "--end_mask %s", val => \&get_safe_word},
		  this_iter => { cmd_arg => "--num_iter %s", val => \&get_safe_integer},
		  pssm_eval => { cmd_arg => "--pssm_evalue %g", val => \&get_safe_number, default_arg=>"--pssm_evalue 1e-3"},
		  lib_abbr => { cmd_arg => "--db=%s", val => \&get_safe_word},
	 },
	 embed_params => 1,
	 www_opts => {
		      hide_align => { arg => \$HIDE_ALIGN, val => \&get_option, cmd_arg=>1},
